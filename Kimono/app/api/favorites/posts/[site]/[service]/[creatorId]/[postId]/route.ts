...
async function mutateFavorite(method: "POST" | "DELETE", input: { site: KimonoSite; service: string; creatorId: string; postId: string; cookie: string }) {
  ...
  try {
    const response = await fetch(`${getBaseUrl(input.site)}/api/v1/favorites/post/${input.service}/${input.creatorId}/${input.postId}`, {
      method,
    ...
    if (!response.ok) {
      throw new Error(`favorite post mutation failed: ${response.status}`);
    }
...
async function persistChronology(site: KimonoSite, service: string, creatorId: string, postId: string, action: "upsert" | "delete") {
  await withDbConnection(async (conn) => {
    ...
    await db.deleteFavoriteChronologyEntry(conn as any, "post", site, service, creatorId, postId);
  });
}

async function handleMutation(method: "POST" | "DELETE", params: { site: string; service: string; creatorId: string; postId: string }) {
  ...
  if (!site || !service || !creatorId || !postId) {
    return NextResponse.json({ error: "Invalid favorite post params" }, { status: 400, headers: { "x-kimono-source": "stale" } });
  }
  ...
  try {
    ...
    const result = await mutateFavorite(method, { site, service, creatorId, postId, cookie });
    if (!result.ok) {
      ...
      return buildRateLimitedResponse(result.retryAfterMs);
    }

    await persistChronology(site, service, creatorId, postId, method === "POST" ? "upsert" : "delete");
    return NextResponse.json({ ok: true }, { headers: { "x-kimono-source": "upstream" } });
  } catch (error) {
    await logAppError("api", "favorite post mutation error", error, {
      details: { route: "/api/favorites/posts/[site]/[service]/[creatorId]/[postId]", method, site: params.site, service, creatorId, postId },
    });
    return NextResponse.json({ error: "Failed" }, { status: 503, headers: { "x-kimono-source": "stale" } });
  }
}

export async function POST(_request: NextRequest, context: { params: Promise<{ site: string; service: string; creatorId: string; postId: string }> }) {
  return handleMutation("POST", await context.params);
}

export async function DELETE(_request: NextRequest, context: { params: Promise<{ site: string; service: string; creatorId: string; postId: string }> }) {
  return handleMutation("DELETE", await context.params);
}
