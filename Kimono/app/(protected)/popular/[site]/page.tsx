...
export default function PopularPage() {
  ...
  const current = Math.floor(offset / 50) + 1;
  // If API doesn't return count, estimate total based on current page + if it's full (50 items)
  const apiCountTotal = Math.ceil((data?.props?.count || 0) / 50);
  const hasFullPage = data?.posts && data.posts.length >= 50;
  const estimatedTotal = Math.max(apiCountTotal, current + (hasFullPage ? 1 : 0));
    ...
    <div className="space-y-6">
      ...
      {period !== "recent" && data?.info?.navigation_dates?.[period] && (
        <div className="flex items-center gap-4">
          <Button
            ...
            onClick={() => {
              setDate(data.info.navigation_dates[period][0]);
              setOffset(0);
            }}
          >
            &larr; Précédent
          ...
          <Button
            ...
            onClick={() => {
              setDate(data.info.navigation_dates[period][1]);
              setOffset(0);
            }}
          >
            Suivant &rarr;
      ...
      {loading ? (
        ...
        <>
          ...
          {/* Corrected Pagination to ensure it shows up regardless of props.count */}
